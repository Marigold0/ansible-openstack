...
----------------------------------------------
0. copy pulic key nya dahulu
    ssh-copy-id user@hostname/ip

    ssh-copy-id init5@172.31.250.19
    
1. Edit File Inventory/host.ini

    [aio]
    { hostname } ansible_host={ internal ip } ansible_user={ user node target }
    optest2 ansible_host=172.31.250.19 ansible_user=init5

2. Untuk setting global ada pada file group_vars/all/vars.yml

3. Secret yang sudah di enkripsi ada di group_vars/all/vault.yml
    *Untuk edit*
...
4. Untuk edit / setting aio ada pada groups_vars/aio/vars.yml
   Jangan lupa masukan nama disk yang akan dijadikan osd

5. Kemudian run playbook dengan *default pass adalah admin
